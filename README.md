...
| 算法技能包 | 4B 已合并并验证 | 保留 `0.1.0`、`0.2.0` 历史，以 `0.2.1` 提供隔离双语言练习、受限 Runner 证据和延迟复测 |
...
| 第五里程碑 5C | 本地实现已完成 | 就业优先实验、分级证据门禁、外部真人评审、用户手动作记录、可选隐私收入记录与学习回流已形成纵向切片；90 项后端、18 项前端、生产构建及桌面/移动 E2E 等本地门禁通过；真实求职动作仍取决于当前用户证据 |
...
4. **练习、项目和掌握评估**
   ...
   - 已保留 `algorithm@0.1.0`、`0.2.0` 历史引用，并新增 `0.2.1` 作为 4B 草稿；
...
5. **变现实验与持续更新**
   ...
   - 5C 已按就业优先边界实施本地实验纵向切片；只有当前用户取得操作 `verified`、保持 `retained`、迁移与作品外部真人通过评审，并绑定当前已接受市场研究后，才能记录其在产品外手动完成的求职动作；
