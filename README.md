...
| 第六里程碑 6A–6D | 6D 已激活、观察中 | 浏览器同源 API 修复精确提交 `c781fe1` 已部署，旧 `5b4af16` 保留为回滚点；真实库 47 表／54 行、策略 `1.1.0`、远程 Runner、身份、Tailnet HTTPS、备份、资源和自动回滚均已复验。云端实际下发脚本不再包含本机 API 地址，页面/API 路由均为 200；Headless 浏览器仍被 Tailscale Serve 关闭连接，真实手机复验、观察期时长和账单实耗仍待确认。真实 AI／来源／邮件、公开访问和扩容继续关闭 |
