...
| 确定性本地检查 | 已通过（2026-07-28） | Node 24 下完整 `release-readiness` 通过，覆盖仓库治理、前后端质量、桌面/移动 E2E、契约漂移和敏感信息扫描 |
| GitHub Actions 基线 | 已验证（2026-07-28） | 第三里程碑远程工作流 12 项检查全部成功 |
...
| 第四里程碑 4A | 本地实现与验证已完成 | 共同主干入口已覆盖学习、练习、追加纠错、六维受限证据和第 1、2、4、7、15 天复习；尚未提交、推送或验证本分支远程 CI |
| 隔离代码运行 | 延后 | 编程练习阶段经确认后再评估 Docker/WSL |
| 互联网部署 | 延后 | 本地验证达标并获得确认后再设计 |

当前 Web 页面提供算法诊断、规划和学习执行的受限草稿预览。规划和确定性练习由本地规则处理；自由文本和代码文本只做结构校验，不能据此判断内容正确，也不代表算法技能包已经激活或用户已经掌握相关能力。

第四里程碑的详细边界见 [`docs/architecture/learning-execution.md`](docs/architecture/learning-execution.md)，后续门禁和实施事项见根目录 [`TODO.md`](TODO.md)。
...
4. **练习、项目和掌握评估**
   - 4A 共同主干入口受限纵向切片已在本地实现并通过发布就绪检查；
