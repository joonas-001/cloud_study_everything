...
| 第六里程碑 6A–6C | 6A、6B 完成；6C 进行中 | 6B 已在新加坡单实例完成 Tailnet-only 合成数据私有预发布、单账号访问、加密备份恢复和预算验收。6C 真实数据库加密副本迁移与回滚演练已通过；远程 Runner 已实现独立 Unix Socket Broker，FastAPI 无 Docker 权限，生产策略仍关闭，正在等待云端十项安全与资源矩阵 |
...
6. **互联网部署评估**
   ...
   - 远程 Runner 的独立 Broker 已实现，云端验证通过前和验证结束后均保持生产关闭；
