...
| GitHub Actions 基线 | 已验证 | 当前远程基线已通过；第三里程碑新增检查需在推送后由 GitHub 再次验证 |
| 技能包治理骨架 | 已完成 | 注册表、manifest、内容哈希、Schema 和依赖图检查 |
| 初版视觉方向 | 已应用于预览 | “静水深流”：克制、清晰、重证据，仍可根据真实使用体验修改 |
| 算法技能包 | 受限草稿预览 | 包含诊断、4 项受监测来源、多专家公开成果和确定性规划模板 |
...
当前 Web 页面提供算法诊断和学习规划的受限草稿预览。规划由本地确定性模拟器生成，不能判断自由文本答案是否正确，也不代表算法技能包已经激活或用户已经掌握相关能力。
