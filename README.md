...
| 第六里程碑 6A–6D | 6D 实施中 | 6B 私有预发布和 6C 真实库演练／远程 Runner 云端矩阵已通过。项目所有者于 2026-08-21 授权实施 6D；当前正在建立 `1.1.0` 启用策略、持久 Broker、完整矩阵和失败回退。生产仍未切换，等待确认已合并 `main@b213a91` 与云端已确认的 7F 提交 `6acdaca` 之间的精确应用基线；当前完整本地门禁已通过 |
...
6. **互联网部署评估**
   ...
   - 远程 Runner 的独立 Broker 已实现并通过云端十项矩阵；项目所有者已于 2026-08-21
     授权 6D，当前正实施版本化启用策略、持久服务、重新验证和失败回退；
   - 正式生产切换尚未执行，仍须确认精确应用基线并逐项通过数据、身份、Runner、预算、
     备份、烟雾和观察期门禁；不公开网站、不扩容、不自动增加费用。
7. **前端体验与信息架构重设计**
