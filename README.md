...
| 第六里程碑 6A–6D | 6D 已激活、观察中 | 独立 6D 基线精确提交 `5b4af16` 已部署；真实库 47 表／54 行一致迁移，策略 `1.1.0`、持久 Broker、live API、十项矩阵、身份、Tailnet HTTPS、备份、资源和回滚点均已验收。观察中发现已部署浏览器脚本错误回退到本机 API；同源 `/api` 修复已通过本地完整门禁，等待新 PR CI 和受控重部署。真实 AI／来源／邮件、公开访问和扩容继续关闭；真实移动浏览器复验、观察期时长和账单实耗仍待确认 |
...
6. **互联网部署评估**
   ...
   - 精确提交 `5b4af16` 和真实库已完成私有切换，身份、Tailnet HTTPS、备份、资源与回滚点
     通过；观察中发现生产浏览器脚本错误回退到 `127.0.0.1:8000`，同源 `/api` 修复及最终
     构建产物防回归门禁已经完成本地验证，等待新 PR CI 和受控重部署；不公开网站、不扩容、
     不自动增加费用。
7. **前端体验与信息架构重设计**
