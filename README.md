...
| 第七里程碑 7A–7F | 已完成并合并 | PR #18 已合并为 `main@90e16e2`；7F 保持零新增依赖完成 Windows 本地与 Linux PR CI 自动验收，项目所有者已确认完成。合并后主线 CI 运行号尚未在仓库证据中记录；Windows 实际高对比度和连续两小时阅读疲劳为非阻断后续人工任务，未执行前不得写成已通过或声称 WCAG 合规认证 |
| 第八里程碑 8A–8G | 8A 已完成；8B 已授权实施 | `main@90e16e2` 合并后 CI `32806434805` 共 22 项成功，8B 基线门禁已解除。8B 只建立通用契约和未开放入口的 `algorithm@0.3.0` 草稿内容底座；不修改历史版本、不创建真实学习记录、不调用外部 AI/来源、不切换入口。8C–8G 未授权 |
...
[`docs/research/github-open-source-gap-analysis.md`](docs/research/github-open-source-gap-analysis.md)。

参考 `xingkongliang/skills-manager` 并按本项目隐私与授权边界适配的 GitHub Issues 流程见
[`docs/research/github-issues-workflow.md`](docs/research/github-issues-workflow.md)。

依据该差距基线制定的第八里程碑方案和已完成 8A 规格见
[`docs/architecture/milestone-8-learning-core.md`](docs/architecture/milestone-8-learning-core.md)。
精确能力图与机器校验对象见
...
6. **互联网部署评估**
   ...
   - 远程 Runner 的独立 Broker 已按策略 `1.1.0` 持久启用并通过激活与独立复测两轮云端
     十项矩阵；FastAPI 仍无 Docker 权限，项目容器残留为零；
   - 浏览器同源 API 修复精确提交 `c781fe1` 已受控部署，旧 `5b4af16` 保持可回滚；最终浏览器
     脚本不再包含 `127.0.0.1:8000`，Tailnet 首页、学习、证据、设置和三条同源 API 均返回
     200；Headless 浏览器仍受 Tailscale Serve 限制，真实手机复验和账单实耗待确认；不公开
     网站、不扩容、不自动增加费用。
7. **前端体验与信息架构重设计**
