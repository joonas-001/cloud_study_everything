...
| 第六里程碑 6A | 已正式交付 | 已确认新加坡单实例、Microsoft 白名单、Tailscale 私有入口、¥50 硬上限、SQLite、禁用远程 Runner、加密备份和 7 天日志；实现 PR #9 与交付修复 PR #10 已合并，PR #10 CI `31301974590` 和合并后主线 CI `31302076612` 均为 22 项成功，本地合成备份恢复及预检演练通过；6B 云资源创建仍未授权 |
| 第七里程碑 7A／7B | 7A 已完成；7B 已实现并验证 | 独立 `codex/milestone-7a` worktree 已完成审计和方向确认，并以零新增依赖实现统一外壳、桌面六区侧栏、移动五项底栏、页面上下文及准确受限的新入口；`Leonxlnx/taste-skill` 仅作只读方法参考，未安装、执行或复制。7C 至 7F 仍须逐阶段确认 |
...
5. **变现实验与持续更新**
   ...
   - 不保证工作、订单或收入。
6. **互联网部署评估**
   - 6A 已确认并在本地实现新加坡单实例、Tailscale 私有访问、Microsoft 单账号
     Web/API 双层白名单、SQLite、远程 Runner 禁用及加密备份恢复边界，正式交付仍待
     提交、远程 CI 与合并；
   - 后续仍依次进行合成数据私有预发布、真实数据迁移演练和所有者单用户上线；
   - 6B、6C、6D 分别单独授权；当前不创建云资源、不公开网站、不产生费用。
7. **前端体验与信息架构重设计**
   ...
   - 收件箱与设置同级；桌面使用统一左侧导航，移动端使用“今日、学习、证据、收件箱、
     更多”底部导航；
   - `Leonxlnx/taste-skill`、GOV.UK Design System、Carbon 和 Primer React 仅作只读方法
     参考，未安装、执行或复制；7C 至 7F 仍须逐阶段确认。
