...
项目所有者随后确认将已验证 Runner 纳入 6D 启用范围。该范围决定不等于 6D 授权；6D
获得最终授权并重新通过上线预检前，Broker 继续停止且不得接入 live API。
...
6C 实现、验证证据和“Runner 纳入 6D 范围”的决定已通过 PR #16 合并到
`main@65529de`。PR CI `31886478208` 与合并后主线 CI `31886598716` 均为 22 项成功；
Broker 和生产策略仍保持关闭，6D 仍等待最后一次单独授权。
