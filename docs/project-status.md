...
- 第八里程碑 M8-D01–D15、8A、8B、8C、8D、8E 与 8F 当前授权范围已完成；8B 已实现通用契约及关闭入口的
  ...
  只复制不提交的允许列表诊断。`0.3.0` 入口仍关闭，默认入口仍为 `0.2.2`；完整本地
  `release-readiness` 已通过 146 项后端测试（1 项按预期跳过）、44 项前端测试、生产构建与
  13 个静态页面、26 项 E2E（6 项按既有范围跳过）、全部治理检查、契约无漂移和密钥扫描；
  8G、GitHub 远程写操作、远程来源复核、第二评审和真实 Runner 全量复验仍未授权；
...
1. 保持 `algorithm@0.3.0` 关闭入口；8G 继续等待单独授权，远程来源实质复核、强制第二评审和锁定镜像 Runner
